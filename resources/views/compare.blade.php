<x-layouts.app title="Compare Products"
    description="Compare Jackpot BD LTD gear side by side — prices, specifications, materials, warranty, and features at a glance.">
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14" data-compare>
        ...
        @if ($pageData->isEmpty())
            <div data-compare-empty class="mx-auto max-w-md py-20 text-center">
                <svg class="mx-auto h-40 w-auto" viewBox="0 0 220 150" fill="none" aria-hidden="true">
                    ...
                    <rect x="66" y="46" width="40" height="56" rx="8" class="fill-white stroke-navy-900"
                        stroke-width="3.5" />
                    <rect x="114" y="46" width="40" height="56" rx="8" class="fill-white stroke-navy-300"
                        stroke-width="3.5" stroke-dasharray="6 6" />
                    <path d="M80 68h12M80 78h12M128 68h12M128 78h12" class="stroke-navy-300" stroke-width="3"
                        stroke-linecap="round" />
                    <path d="M170 34c3.3 0 6-2.7 6-6 0 3.3 2.7 6 6 6-3.3 0-6 2.7-6 6 0-3.3-2.7-6-6-6Z"
                        class="fill-bronze-400" />
        ...
        @else
            {{-- Comparison table --}}
            <div class="mt-8 overflow-x-auto rounded-card bg-surface shadow-card lg:overflow-x-visible"
                data-compare-scroller>
                <table class="w-full min-w-190 border-collapse text-sm" data-compare-table>
                    ...
                    <thead>
                        <tr>
                            <th scope="col"
                                class="sticky top-16 left-0 z-30 w-44 border-b border-navy-100 bg-surface/95 p-5 text-left align-bottom backdrop-blur-sm lg:top-18 lg:w-52">
                                <span
                                    class="text-xs font-semibold tracking-wide text-navy-500 uppercase">Comparing</span>
                            </th>
                                    ...
                                    <div class="flex items-start gap-3">
                                        @if ($column->image)
                                            <img src="{{ $column->image }}" alt="" loading="lazy"
                                                class="hidden size-14 shrink-0 rounded-xl object-cover sm:block">
                                        @endif
                                        <div class="min-w-0 flex-1">
                                            <p class="truncate font-display text-sm font-bold text-navy-900">
                                                {{ $column->name }}</p>
                                            <p class="mt-0.5 font-display text-base font-extrabold text-navy-900">
                                                {{ $column->price }}</p>
                                            <div class="mt-2 flex items-center gap-2">
                                                <x-ui.button size="sm" class="px-3! py-1.5! text-xs!"
                                                    data-add-to-cart data-product-id="{{ $column->product->id }}">
                                                    Add to cart
                                                </x-ui.button>
                    ...
                    <tbody>
                        ...
                        <tr data-compare-row>
                            <th scope="row"
                                class="sticky left-0 z-10 bg-surface p-5 text-left align-top font-semibold text-navy-900">
                                Image</th>
                            @foreach ($pageData->columns as $index => $column)
                                <td data-col="{{ $index }}" class="p-5 align-top">
                                    <a href="{{ $column->url }}" class="group block overflow-hidden rounded-card">
                                        ...
                                        @else
                                            <div
                                                class="flex aspect-4/3 w-full items-center justify-center bg-navy-100 text-navy-400">
                                                No image</div>
                                        @endif
                                    </a>
                                </td>
                        ...
                        <tr data-compare-row class="border-t border-navy-100">
                            ...
                            @foreach ($pageData->columns as $index => $column)
                                <td data-col="{{ $index }}" data-compare-value class="p-5 align-top">
                                    <span
                                        class="font-display text-lg font-extrabold text-navy-900">{{ $column->price }}</span>
                                    @if ($column->oldPrice)
                            ...
                            @foreach ($pageData->columns as $index => $column)
                                <td data-col="{{ $index }}" data-compare-value class="p-5 align-top">
                                    <span class="flex items-center gap-2">
                                        ...
                                        <span class="text-navy-600">{{ $column->rating }}
                                            ({{ $column->reviews }})</span>
                                    </span>
                                </td>
                            @endforeach
                            ...
                            @foreach ($pageData->columns as $index => $column)
                                <td data-col="{{ $index }}" data-compare-value class="p-5 align-top">
                                    <span @class([
                                        ...
                                        'text-green-700' =>
                                            !$column->isLowStock && $column->availability !== 'Out of stock',
                        ...
                        @if ($pageData->columns->contains(fn($column) => $column->brand))
                            <tr data-compare-row class="border-t border-navy-100">
                                <th scope="row"
                                    ...
                                    Brand</th>
                                @foreach ($pageData->columns as $index => $column)
                                    <td data-col="{{ $index }}" data-compare-value
                                        class="p-5 align-top text-navy-600">{{ $column->brand ?? '—' }}</td>
                                @endforeach
                            </tr>
                        @endif
                        ...
                        @if ($pageData->columns->contains(fn($column) => $column->category))
                            <tr data-compare-row class="border-t border-navy-100">
                                ...
                                @foreach ($pageData->columns as $index => $column)
                                    <td data-col="{{ $index }}" data-compare-value
                                        class="p-5 align-top text-navy-600">{{ $column->category ?? '—' }}</td>
                                @endforeach
                            </tr>
                        @endif
                        ...
                        @if (count($pageData->specificationLabels) > 0)
                            ...
                            @foreach ($pageData->specificationLabels as $label)
                                <tr data-compare-row class="border-t border-navy-100">
                                    ...
                                    @foreach ($pageData->columns as $index => $column)
                                        <td data-col="{{ $index }}" data-compare-value
                                            class="p-5 align-top text-navy-600">
                        ...
                        @if ($pageData->hasMaterials())
                            ...
                            <tr data-compare-row class="border-t border-navy-100">
                                ...
                                @foreach ($pageData->columns as $index => $column)
                                    <td data-col="{{ $index }}" data-compare-value
                                        class="p-5 align-top leading-relaxed text-navy-600">
                                    ...
                                    <td data-col="{{ $index }}" data-compare-value class="p-5 align-top">
                                        @if ($column->warranty)
                                            <span class="inline-flex items-center gap-2 text-navy-700">
                                                <svg class="size-4 shrink-0 text-olive-600" viewBox="0 0 24 24"
                        ...
                        <tr class="border-t border-navy-100">
                            <th scope="row" class="sticky left-0 z-10 bg-surface p-5"><span
                                    class="sr-only">Actions</span></th>
                            @foreach ($pageData->columns as $index => $column)
                                <td data-col="{{ $index }}" class="p-5 align-top">
                                    <div class="flex flex-col gap-2">
                                        <x-ui.button variant="accent" size="sm" data-add-to-cart
                                            data-product-id="{{ $column->product->id }}">
                                            Add to cart
                                        </x-ui.button>
                                        <x-ui.button :href="$column->url" variant="outline" size="sm">View
                                            details</x-ui.button>
                                    </div>
                                </td>
                            @endforeach
