...
<x-layouts.app title="Wishlist"
    description="The gear you've saved for later at Jackpot BD LTD — keep an eye on availability and move favorites to your cart.">
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14" data-wishlist>
        ...
        <div class="grid grid-cols-1 gap-10 lg:grid-cols-4">
            ...
            <div class="lg:col-span-3">
                ...
                <div class="mt-4 flex flex-wrap items-end justify-between gap-4">
                    <div>
                        <div class="flex flex-wrap items-baseline gap-3">
                            ...
                            <p class="text-navy-500" data-wishlist-count-label>{{ $summary->itemCount }} saved
                                {{ $summary->itemCount === 1 ? 'item' : 'items' }}</p>
                        </div>
                        <p class="mt-2 max-w-xl text-navy-600">Saved gear stays here so you never lose sight of it.
                            We'll flag anything running low.</p>
                    </div>
                    <div class="flex flex-wrap gap-3" data-wishlist-actions
                        @if ($summary->isEmpty()) hidden @endif>
                ...
                {{-- Grid --}}
                <ul data-wishlist-grid class="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3"
                    @if ($summary->isEmpty()) hidden @endif>
                    @foreach ($summary->items as $line)
                        ...
                        @endphp
                        <li data-wishlist-item data-wishlist-item-id="{{ $line->wishlistItem->id }}"
                            data-product-id="{{ $product->id }}" data-availability="{{ $availability }}"
                            ...
                            <div class="relative aspect-4/3 overflow-hidden bg-navy-100">
                                <a href="{{ route('product.show', $product) }}" class="block size-full">
                                    ...
                                    @else
                                        <div
                                            ...
                                            <svg class="size-14 text-navy-300" viewBox="0 0 24 24" fill="none"
                                ...
                                <button type="button" data-wishlist-remove
                            ...
                            <div class="flex flex-1 flex-col p-6">
                                @if ($product->category)
                                    <p class="text-xs font-semibold tracking-wide text-bronze-600 uppercase">
                                ...
                                <p class="mt-3 flex items-baseline gap-2">
                                    <span
                                        class="font-display text-xl font-extrabold text-navy-900">{{ $product->formattedPrice() }}</span>
                                    @if ($product->isOnSale())
                                        <span
                                            class="text-sm text-navy-400 line-through">{{ $product->formattedCompareAtPrice() }}</span>
                                    @endif
                                </p>
                                ...
                                <p class="mt-2 flex items-center gap-2 text-sm font-medium {{ $meta['text'] }}">
                                    <span class="relative flex size-2" aria-hidden="true">
                                        @if ($availability !== 'out-of-stock')
                                            <span
                                                class="absolute inline-flex size-full animate-ping rounded-full {{ $meta['dot'] }} opacity-60"></span>
                                        @endif
                                        <span
                                            class="relative inline-flex size-2 rounded-full {{ $meta['dot'] }}"></span>
                                    </span>
                                    {{ $meta['label'] }}
                                </p>
                                ...
                                <div class="mt-5 flex gap-2 pt-1">
                                    ...
                                    @else
                                        <x-ui.button class="flex-1" data-wishlist-move-to-cart
                                            data-wishlist-item-id="{{ $line->wishlistItem->id }}"
                                    ...
                                    <button type="button" data-compare-toggle data-product-id="{{ $product->id }}"
                ...
                <div data-wishlist-empty @class([
                    ...
                    <svg class="mx-auto h-44 w-auto" viewBox="0 0 220 160" fill="none" aria-hidden="true">
                        ...
                        <path d="M110 112s-30-19-38-36a22 22 0 0 1 38-22 22 22 0 0 1 38 22c-8 17-38 36-38 36Z"
                            class="fill-white stroke-navy-900" stroke-width="3.5" stroke-linecap="round"
                            stroke-linejoin="round" />
                        <path d="M95 62c2-6 8-9 14-8" class="stroke-navy-300" stroke-width="3"
                            stroke-linecap="round" />
                        <path
                            d="M162 38c3.6 0 6.5-2.9 6.5-6.5 0 3.6 2.9 6.5 6.5 6.5-3.6 0-6.5 2.9-6.5 6.5 0-3.6-2.9-6.5-6.5-6.5Z"
                            class="fill-bronze-400" />
                        <path d="M48 116c2.8 0 5-2.2 5-5 0 2.8 2.2 5 5 5-2.8 0-5 2.2-5 5 0-2.8-2.2-5-5-5Z"
                    ...
                    <p class="mt-3 leading-relaxed text-navy-600">
                        Tap the heart on any product to save it here. Your favorites will be waiting whenever you're
                        ready.
                    </p>
