<x-layouts.admin :title="$title" :breadcrumbs="$breadcrumbs">
    ...
    <x-admin.page-header title="Banner Promotions" description="Hero banners and storefront callouts.">
        <x-slot:actions>
            @if (auth('admin')->user()?->hasPermission('promotions.manage'))
                <x-admin.button :href="route('admin.banner-promotions.create')" size="sm">
    ...
    <x-admin.data-table>
        <x-slot:toolbar>
            <form method="GET" action="{{ route('admin.banner-promotions.index') }}" class="flex w-full gap-2 lg:ml-auto lg:w-auto">
        ...
        <thead>
            <tr class="border-b admin-border bg-admin-bg/40">
                <th scope="col" class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider admin-muted">Banner</th>
                ...
                <tr class="group transition-colors hover:bg-admin-bg/60">
                    ...
                    <td class="px-6 py-4 text-sm admin-text-secondary">
                        @if ($promotion->starts_at || $promotion->ends_at)
                            {{ $promotion->starts_at?->format('M j') ?? 'Any' }} → {{ $promotion->ends_at?->format('M j, Y') ?? 'No end' }}
                        @else
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-1">
                            @if (auth('admin')->user()?->hasPermission('promotions.manage'))
                                <x-admin.button variant="ghost" size="icon-sm" :href="route('admin.banner-promotions.edit', $promotion)" title="Edit" aria-label="Edit {{ $promotion->name }}">
                                ...
                                <form method="POST" action="{{ route('admin.banner-promotions.destroy', $promotion) }}" onsubmit="return confirm('Delete this banner?')">
                                    @csrf
                                    @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-5 py-14">
                        <x-admin.empty-state
                            title="No banner promotions"
                            description="Create hero banners for the homepage and shop."
                            action-label="Add banner"
                            :action-href="route('admin.banner-promotions.create')"
                        />
                    </td>
                </tr>
            @endforelse
