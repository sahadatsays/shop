...
<x-layouts.admin :title="$title" :breadcrumbs="$breadcrumbs">
    ...
    <div
        x-data="purchaseCreateForm({
            ...
            shippingCents: {{ (int) old('shipping_cents', $purchase->shipping_cents) }},
            discountCents: {{ (int) old('discount_cents', $purchase->discount_cents) }},
            taxCents: {{ (int) old('tax_cents', $purchase->tax_cents) }},
            initialItems: @js($initialItems),
        })"
    >
        <x-admin.page-header :title="'Edit '.$purchase->purchase_number" description="Draft purchases can be edited before approval.">
            <x-slot:actions>
                <x-admin.button variant="secondary" size="sm" :href="route('admin.purchases.show', $purchase)">Back</x-admin.button>
            </x-slot:actions>
        </x-admin.page-header>

        <form method="POST" action="{{ route('admin.purchases.update', $purchase) }}" class="grid gap-6 xl:grid-cols-3" @submit="return onSubmit()">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6 xl:col-span-2">
                <x-admin.form-card title="Purchase details">
                    <div class="grid gap-5 sm:grid-cols-2">
                        ...
                        <x-admin.input label="Purchase date" name="purchase_date" type="date" :value="old('purchase_date', $purchase->purchase_date?->toDateString())" required />
                        <x-admin.input label="Expected delivery" name="expected_delivery_date" type="date" :value="old('expected_delivery_date', $purchase->expected_delivery_date?->toDateString())" />
                        <x-admin.textarea label="Notes" name="notes" :value="old('notes', $purchase->notes)" class="sm:col-span-2" rows="3" />
                    </div>
                </x-admin.form-card>
            ...
            <div class="space-y-6">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <x-admin.button variant="ghost" :href="route('admin.purchases.show', $purchase)">Cancel</x-admin.button>
                </div>
            </div>
        </form>
    </div>
