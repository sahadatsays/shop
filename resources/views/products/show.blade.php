...
<x-layouts.app :title="$pageTitle" :description="$pageDescription">
        ...
        <ol class="flex flex-wrap items-center gap-2 text-sm text-navy-500">
            @foreach ($breadcrumbs as $crumb)
                ...
                <li>
                    @if ($crumb['url'])
                        <a href="{{ $crumb['url'] }}"
                            class="transition-colors duration-200 hover:text-navy-900">{{ $crumb['label'] }}</a>
                    @else
    ...
    {{-- Product section --}}
    <section class="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:px-8"
        data-gallery data-product-page data-product-id="{{ $product->id }}">
            ...
            <div data-stage
                ...
                @if ($galleryImages->isNotEmpty())
                    @foreach ($galleryImages as $image)
                        <div data-art="{{ $loop->index }}" @if (!$loop->first) hidden @endif
                ...
                @else
                    <div data-art="0"
                        class="absolute inset-0 transition-transform duration-200 ease-out will-change-transform">
                        <div
                            class="flex size-full flex-col items-center justify-center gap-4 bg-linear-to-br from-navy-200 via-navy-100 to-bronze-100">
                            <svg class="size-24 text-navy-400/70" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"
                                aria-hidden="true">
                                <path
                                    d="M16 3l5 3-2 5-2-1v10a1 1 0 0 1-1 1H8a1 1 0 0 1-1-1V10l-2 1-2-5 5-3a4 4 0 0 0 8 0Z" />
                            </svg>
                            <p class="text-sm font-medium tracking-wide text-navy-500 uppercase">{{ $product->name }}
                            </p>
                        </div>
                    </div>
                @endif
        ...
        <div class="flex flex-col">
            @if ($product->brand)
                <p class="text-sm font-semibold tracking-widest text-bronze-600 uppercase">{{ $product->brand->name }}
                </p>
            @endif
            ...
            <div class="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm">
                <span @class([
                    ...
                    @if (!$product->isOutOfStock())
                        <span class="relative flex size-2.5">
                            <span
                                class="absolute inline-flex size-full animate-ping rounded-full bg-green-500 opacity-60"></span>
                ...
                @if ($product->sku)
                    <span class="text-navy-500">SKU: <span
                            class="font-medium text-navy-700">{{ $product->sku }}</span></span>
                @endif
            </div>
            ...
            @if ($colors->isNotEmpty())
                <fieldset class="mt-8" data-option-group>
                    <legend class="text-sm font-semibold text-navy-900">
                        Color: <span data-option-label
                            class="font-normal text-navy-600">{{ $colors->first()->value }}</span>
                    </legend>
                    <div class="mt-3 flex flex-wrap gap-3">
                        @foreach ($colors as $color)
                            <button type="button" data-option-value="{{ $color->value }}"
                                ...
                                <span
                                    class="size-9 rounded-full {{ $product->colorSwatchClass($color->value) }} shadow-inner"></span>
                            </button>
                        @endforeach
                    </div>
                </fieldset>
            ...
            @if ($sizes->isNotEmpty())
                <fieldset class="mt-8" data-option-group>
                    <div class="flex items-center justify-between">
                        <legend class="text-sm font-semibold text-navy-900">
                            Size: <span data-option-label
                                class="font-normal text-navy-600">{{ $sizes->first()->value }}</span>
                        </legend>
                        <a href="#"
                            class="text-sm text-olive-700 underline-offset-4 transition-colors duration-200 hover:text-olive-800 hover:underline">Size
                            guide</a>
                    </div>
            ...
            <div class="mt-8" data-atc-anchor>
                <div class="flex flex-wrap items-stretch gap-3">
                    <div class="flex items-center rounded-xl border border-navy-200 bg-surface" data-quantity>
                        ...
                        <input type="number" data-qty-input value="1" min="1"
                            max="{{ max(1, $product->stock_quantity) }}" inputmode="numeric" aria-label="Quantity"
                            @disabled($product->isOutOfStock())
                    ...
                    <x-ui.button variant="accent" class="flex-1" data-add-to-cart
                        data-product-id="{{ $product->id }}" :disabled="$product->isOutOfStock()">
                ...
                <div class="mt-3 flex flex-wrap items-stretch gap-3">
                    ...
                    <x-ui.button variant="outline"
                        ...
                        aria-pressed="{{ $productInWishlist ? 'true' : 'false' }}" @class([
                            'px-4',
                            'border-bronze-500! bg-bronze-50! text-bronze-700!' => $productInWishlist,
                        ])>
                    ...
                    <x-ui.button variant="outline"
                        ...
                        data-compare-toggle data-product-id="{{ $product->id }}"
                        aria-pressed="{{ $productInCompare ? 'true' : 'false' }}" @class([
                            'px-4',
                            'border-olive-600! bg-olive-50! text-olive-700!' => $productInCompare,
                        ])>
                        <svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                            stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
            ...
            <div class="mt-6">
                ...
                <ul class="mt-3 flex flex-wrap gap-2">
                    @foreach (['VISA', 'Mastercard', 'AMEX', 'PayPal', 'Apple Pay', 'G Pay'] as $method)
                        <li
                            class="rounded-lg border border-navy-200 bg-surface px-3 py-1.5 text-xs font-bold tracking-wide text-navy-700">
                            {{ $method }}</li>
                    @endforeach
                </ul>
            </div>
    ...
    <section id="reviews" class="bg-surface py-20" data-reveal>
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div
                    class="mt-6 rounded-card border border-green-200 bg-green-50 px-5 py-4 text-sm font-medium text-green-800">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <div class="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-3">
                <div class="rounded-card bg-canvas p-8 lg:self-start">
                    ...
                    <div class="mt-3 flex items-center gap-2">
                        <x-ui.rating :value="$displayRating" />
                        <span class="text-sm text-navy-500">{{ $displayReviewCount }}
                            {{ Str::plural('review', $displayReviewCount) }}</span>
                    </div>
                        ...
                        @foreach ($reviewSummary['distribution'] as $stars => $sharePercent)
                            <div class="flex items-center gap-3 text-sm">
                                ...
                                <div class="h-2 flex-1 overflow-hidden rounded-full bg-navy-100">
                                    <div class="h-full rounded-full bg-bronze-500"
                                        style="width: {{ $sharePercent }}%"></div>
                                </div>
                    ...
                    @if ($customer)
                        @if ($canReview)
                            <div class="mt-8 rounded-card border border-navy-100 bg-surface p-5 shadow-soft">
                                ...
                                <p class="mt-1 text-sm text-navy-500">Share your experience with this verified
                                    purchase.</p>
                                <form method="POST" action="{{ route('product.reviews.store', $product) }}"
                                    class="mt-5 space-y-5">
                                    @csrf
                                    <input type="hidden" name="redirect" value="product">
                                    <x-account.review-form-fields rating-picker-id="product-rating"
                                        title-id="product-review-title" body-id="product-review-body" />
                                    <x-ui.button variant="secondary" type="submit" class="w-full">Submit
                                        review</x-ui.button>
                                </form>
                            </div>
                        @elseif ($hasReviewed)
                            <p class="mt-8 rounded-xl bg-olive-50 px-4 py-3 text-sm text-olive-800">
                                ...
                                <a href="{{ route('account.reviews') }}"
                                    class="font-semibold underline-offset-4 hover:underline">Manage your reviews</a>
                            </p>
                        @else
                            <p class="mt-8 rounded-xl bg-navy-50 px-4 py-3 text-sm text-navy-600">
                                Reviews open after your order is delivered.
                                <a href="{{ route('account.orders') }}"
                                    class="font-semibold text-navy-900 underline-offset-4 hover:underline">View
                                    orders</a>
                            </p>
                        @endif
                    @else
                        <x-ui.button :href="route('login')" variant="outline" class="mt-8 w-full">Sign in to write a
                            review</x-ui.button>
                    @endif
                </div>
                ...
                <div class="space-y-6 lg:col-span-2">
                        ...
                        <article class="rounded-card bg-canvas p-6 sm:p-8">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                <div class="flex items-center gap-3">
                                    <span
                                        class="flex size-10 items-center justify-center rounded-full bg-navy-900 font-display text-sm font-bold text-bronze-400">
                                        {{ $review->initials() }}
                                    </span>
                                    <div>
                                        ...
                                        <p class="text-xs text-navy-500">{{ $review->created_at?->format('M j, Y') }}
                                        </p>
                                    </div>
                                </div>
                            ...
                            @if ($review->title)
                                <h3 class="mt-4 font-display text-base font-semibold text-navy-900">
                                    {{ $review->title }}</h3>
                            @endif
                    ...
                    @empty
                        <div class="rounded-card bg-canvas p-8 text-center">
                            <p class="text-sm text-navy-600">No reviews yet. Be the first to share your experience
                                after delivery.</p>
                        </div>
                    @endforelse
                </div>
        ...
        <section class="mx-auto max-w-7xl px-4 pb-4 sm:px-6 lg:px-8" data-reveal>
            ...
            <div class="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
                @foreach ($recentlyViewedProducts as $viewedProduct)
                    <x-ui.product-card :name="$viewedProduct->name" :brand="$viewedProduct->brand?->name" :category="$viewedProduct->category?->name" :price="$viewedProduct->formattedPrice()"
                        :old-price="$viewedProduct->isOnSale() ? $viewedProduct->formattedCompareAtPrice() : null" :image="$viewedProduct->primaryImageUrl()" :href="route('product.show', $viewedProduct)" :product-id="$viewedProduct->id" />
                @endforeach
            </div>
        </section>
    @endif
        ...
        <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
            <div class="flex min-w-0 items-center gap-3">
                <span class="hidden size-12 shrink-0 overflow-hidden rounded-xl bg-navy-100 sm:flex">
                    ...
                    @else
                        <span
                            class="flex size-full items-center justify-center bg-linear-to-br from-navy-200 via-navy-100 to-bronze-100">
                            <svg class="size-6 text-navy-400/70" viewBox="0 0 24 24" fill="none"
            ...
            <x-ui.button variant="accent" size="sm" class="shrink-0" data-add-to-cart
                data-product-id="{{ $product->id }}" :disabled="$product->isOutOfStock()">
