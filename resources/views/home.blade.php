...
<x-layouts.app :description="$homepageSettings->meta_description">
        ...
        <section class="border-b border-bronze-500/20 bg-navy-900" data-reveal>
            <div
                ...
                <div class="text-center sm:text-left">
                    ...
                    <h2 class="mt-1 font-display text-xl font-bold text-white sm:text-2xl">
                        {{ $countdownPromotion->headline }}</h2>
    ...
    @if ($enabled->contains('categories') && $categories->isNotEmpty())
        <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28" data-reveal>
            <x-ui.section-heading title="Featured category"
                subtitle="Premium packaging and printed essentials, customized to reflect your brand and make every presentation memorable." />

            <div class="mt-12 grid grid-cols-2 gap-4 sm:gap-6 lg:grid-cols-4">
                @foreach ($categories as $category)
                    <x-ui.category-card :name="$category->name" :count="(int) $category->products_count" :href="route('shop', ['category' => [$category->slug]])" :icon="$categoryIcons[$category->slug] ?? 'M3 11 12 3l9 8M6 10v10h12V10'"
    ...
    @if ($enabled->contains('featured_collections'))
        <section class="bg-surface py-20 lg:py-28" data-reveal>
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <x-ui.section-heading title="Featured collections" />
                ...
                <div class="mt-12 grid grid-cols-1 gap-6 lg:grid-cols-3">
                    @forelse ($featuredCollections as $index => $collection)
                        ...
                        <a href="{{ route('collections.show', $collection) }}" @class([
                            ...
                            <span
                                class="absolute inset-0 bg-linear-to-t from-navy-950/85 via-navy-950/25 to-transparent"
                                aria-hidden="true"></span>
                            <span class="relative p-8">
                                <span
                                    class="block font-display text-2xl font-bold text-white">{{ $collection->name }}</span>
                                @if ($collection->description)
                                    <span class="mt-2 block text-sm text-navy-200">{{ $collection->description }}</span>
                                @endif
                                <span
                                    class="mt-5 inline-flex items-center gap-2 rounded-xl bg-white/15 px-4 py-2.5 text-sm font-semibold text-white backdrop-blur-sm transition-colors duration-300 group-hover:bg-bronze-500">
                                    Explore collection
                                    <svg class="size-4 transition-transform duration-300 group-hover:translate-x-1"
                    ...
                    @empty
                        <p class="col-span-full text-center text-navy-600">Featured collections will appear here once
                            published.</p>
                    @endforelse
                </div>
            </div>
    ...
    @if ($enabled->contains('featured_products') && $featuredProducts->isNotEmpty())
        <section id="featured-products" class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28" data-reveal>
            <div class="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
                <x-ui.section-heading align="left" title="Featured products"
    ...
    @if ($enabled->contains('new_arrivals') && $newArrivals->isNotEmpty())
        <section id="new-arrivals" class="bg-surface py-20 lg:py-28" data-reveal>
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
                    <x-ui.section-heading align="left" title="New arrivals"
    ...
    @if ($enabled->contains('best_sellers'))
        <section id="best-sellers" class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28" data-reveal>
            <div class="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
                <x-ui.section-heading align="left" title="Top Selling Products"
                    subtitle="Ranked from completed customer orders" />
        ...
        <section class="bg-navy-900 py-20 lg:py-24" data-reveal>
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="mx-auto max-w-2xl text-center">
                    <p class="text-sm font-semibold tracking-widest text-bronze-400 uppercase">A standard worth
                        defending</p>
                    <h2 class="mt-3 font-display text-3xl font-bold text-white sm:text-4xl uppercase">Why shop with us
                    </h2>
                </div>
                ...
                <div class="mt-14 grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-5">
                    @foreach ($whyShopFeatures as $feature)
                        <div class="flex flex-col items-center text-center">
                            <span
                                class="flex size-16 items-center justify-center rounded-2xl bg-white/5 text-bronze-400 ring-1 ring-white/10 transition-transform duration-300 hover:scale-110">
    ...
    {{-- Our story — preserved static brand story --}}
        ...
        <div class="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-20">
            <div class="relative">
                ...
                <div class="glass absolute -right-4 -bottom-6 max-w-56 rounded-card p-5 sm:-right-8">
                    ...
                    <p class="mt-1 text-sm text-navy-600">of service before founding Jackpot BD LTD</p>
                </div>
            </div>

            <div>
                <p class="text-sm font-semibold tracking-widest text-bronze-600 uppercase">Our story</p>
                <h2 class="mt-3 font-display text-3xl font-bold text-navy-900 sm:text-4xl">From the 75th Ranger Regiment
                    to the workshop floor</h2>
                ...
                <div class="mt-10 flex flex-wrap gap-4">
                    ...
                    <x-ui.button :href="route('support') . '#team'" variant="ghost">
                        Meet the team
    ...
    @if ($saleProducts->isNotEmpty())
        <section class="relative overflow-hidden bg-navy-950 py-20 lg:py-28" data-reveal>
            <div class="absolute -top-32 right-0 size-96 rounded-full bg-bronze-500/10 blur-3xl" aria-hidden="true">
            </div>
                ...
                <div class="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">
                    @foreach ($saleProducts->take(3) as $product)
                        <a href="{{ route('product.show', $product) }}"
                            class="group overflow-hidden rounded-card bg-white/5 ring-1 ring-white/10 transition-all duration-300 ease-out hover:-translate-y-1 hover:bg-white/10">
                            ...
                            <span class="block p-6">
                                <span
                                    class="block font-display text-lg font-bold text-white">{{ $product->name }}</span>
                                <span class="mt-1 block text-sm text-navy-300">{{ $product->category?->name }}</span>
                                <span
                                    class="mt-4 block text-lg font-bold text-bronze-400">{{ $product->formattedPrice() }}</span>
                            </span>
                        </a>
                    @endforeach
                </div>
    ...
    @if ($enabled->contains('reviews') && $reviews->isNotEmpty())
        <section class="py-20 lg:py-28" data-reveal>
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
                    <x-ui.section-heading align="left" title="Customer reviews" />
                ...
                <div data-carousel
                    class="scrollbar-none -mx-4 mt-12 flex snap-x snap-mandatory gap-6 overflow-x-auto scroll-smooth px-4 pb-4 sm:mx-0 sm:px-0"
                    aria-label="Customer reviews carousel">
                    @foreach ($reviews as $review)
                        <x-ui.review-card :author="$review->author_name" :role="$review->title" :rating="$review->rating" :body="$review->body"
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <ul class="mt-8 flex flex-wrap items-center justify-center gap-x-14 gap-y-6">
                    @foreach ($brands as $brand)
                        <x-ui.brand-logo :name="$brand->name" :logo="$brand->logoUrl()" :href="route('shop', ['brand' => [$brand->slug]])" />
                    @endforeach
                </ul>
            </div>
        </section>
