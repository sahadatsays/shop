...
<x-layouts.app :title="$title"
    description="Every Jackpot BD LTD order in one place — track shipments, download invoices, and reorder your favorites.">
        ...
        <div class="grid grid-cols-1 gap-10 lg:grid-cols-4">
            ...
            <div class="lg:col-span-3">
                ...
                <div class="mt-4 flex flex-wrap items-baseline gap-3">
                    ...
                    <p class="text-navy-500" data-orders-count>{{ $ordersCount }}
                        {{ $ordersCount === 1 ? 'order' : 'orders' }}</p>
                </div>

                {{-- Status filters --}}
                <div class="mt-6 flex flex-wrap gap-2" role="group" aria-label="Filter orders by status">
                    @foreach ($filters as $filter)
                        <button type="button" data-orders-filter="{{ $filter }}"
                ...
                <div class="mt-8 space-y-6">
                    @foreach ($orders as $order)
                        <article data-order-card data-status="{{ $order['status'] }}"
                            ...
                            {{-- Card header --}}
                            <header
                                class="flex flex-wrap items-center gap-x-6 gap-y-3 border-b border-navy-100 bg-navy-50/60 px-7 py-5">
                                <div>
                                    ...
                                    <p class="font-display text-base font-bold text-navy-900">
                                        <a href="{{ $order['track_url'] }}"
                                            class="transition-colors duration-200 hover:text-olive-700">{{ $order['number'] }}</a>
                                    </p>
                                </div>
                                <div>
                            ...
                            <div class="px-7 py-6">
                                ...
                                <ol class="flex items-center" aria-label="Order progress: {{ $order['status'] }}">
                                    @foreach ($steps as $stepIndex => $step)
                                        ...
                                        @if (!$loop->first)
                                            <li aria-hidden="true"
                                                class="h-0.5 flex-1 -translate-y-2.5 rounded-full sm:-translate-y-3 {{ $stepNumber <= $order['progress'] ? 'bg-olive-600' : 'bg-navy-200' }}">
                                            </li>
                                        @endif
                                        <li class="flex flex-col items-center gap-1.5 px-1 sm:px-2">
                                            <span
                                                class="flex size-6 items-center justify-center rounded-full sm:size-7
                                                ...
                                                @else
                                                    <span class="size-1.5 rounded-full bg-navy-300"
                                                        aria-hidden="true"></span>
                                                @endif
                                            </span>
                                            <span
                                                class="text-[0.65rem] font-semibold whitespace-nowrap sm:text-xs {{ $stepNumber <= $order['progress'] ? 'text-navy-900' : 'text-navy-400' }}">{{ $step }}</span>
                                        </li>
                                    @endforeach
                                </ol>
                                ...
                                <div class="mt-6 flex flex-wrap items-center gap-6 border-t border-navy-100 pt-6">
                                    <ul class="flex items-center gap-3" aria-label="Items in this order">
                                        ...
                                        @endforeach
                                        <li class="text-sm text-navy-500">{{ count($order['items']) }}
                                            {{ count($order['items']) === 1 ? 'item' : 'items' }}</li>
                                    </ul>
                ...
                <div data-orders-empty hidden class="mx-auto max-w-md py-16 text-center">
                    <svg class="mx-auto h-32 w-auto" viewBox="0 0 200 140" fill="none" aria-hidden="true">
                        ...
                        <rect x="66" y="44" width="68" height="48" rx="8"
                            class="fill-white stroke-navy-900" stroke-width="3.5" />
                        <path d="M66 58h68M80 72h24" class="stroke-navy-300" stroke-width="3"
                            stroke-linecap="round" />
                        <path
                            d="M152 30c3 0 5.5-2.5 5.5-5.5 0 3 2.5 5.5 5.5 5.5-3 0-5.5 2.5-5.5 5.5 0-3-2.5-5.5-5.5-5.5Z"
