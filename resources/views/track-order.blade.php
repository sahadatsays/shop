<x-layouts.app :title="$title" description="Track your Jackpot BD LTD order with your order number and email address.">
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div class="mx-auto mt-10 max-w-xl">
            ...
            @if ($errors->any())
                <div class="mt-6 rounded-card border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"
                    role="alert">
                    {{ $errors->first() }}
                </div>
            @endif

            <form method="POST" action="{{ route('track-order.store') }}"
                class="mt-8 space-y-5 rounded-card bg-surface p-7 shadow-soft">
                @csrf
            ...
            <p class="mt-6 text-center text-sm text-navy-500">
                Signed in?
                <a href="{{ route('account.orders') }}" class="font-semibold text-navy-900 hover:text-bronze-700">View
