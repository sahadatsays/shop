...
<x-layouts.app title="Saved Addresses" description="Manage shipping and billing addresses for your Valor Supply Co. account.">
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14" data-addresses>
            ...
            <div class="lg:col-span-3">
                ...
                <div class="mt-4 flex flex-wrap items-end justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-navy-600">Where we ship your gear — add, edit, or set your default delivery location.</p>
                    </div>
                ...
                <p class="mt-6 text-sm text-navy-500" data-address-count aria-live="polite">{{ count($addresses) }} saved {{ Str::plural('address', count($addresses)) }}</p>
                ...
                <div class="mt-4 grid grid-cols-1 gap-6 sm:grid-cols-2" data-address-grid>
                        ...
                        >
                            ...
                            <div class="relative h-36 overflow-hidden bg-navy-50" aria-hidden="true">
                                <svg viewBox="0 0 400 144" class="block h-full w-full" preserveAspectRatio="xMidYMid slice">
                                    ...
                                    <path d="M200 28c-14 0-25 11-25 25 0 18 25 40 25 40s25-22 25-40c0-14-11-25-25-25Z" class="fill-bronze-500"/>
                                    <circle cx="200" cy="53" r="7" class="fill-white"/>
                                </svg>
                                <div class="absolute inset-x-0 bottom-0 flex items-center justify-between bg-linear-to-t from-navy-900/70 to-transparent px-4 py-3">
                                    <span class="text-xs font-medium text-white/90">Map preview</span>
                                    <span class="rounded-md bg-white/15 px-2 py-0.5 text-[0.65rem] font-semibold tracking-wide text-white uppercase backdrop-blur-sm">Maps API</span>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-1 flex-col p-6">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="flex items-center gap-3">
                                        <span class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-navy-900/5 text-navy-700">
                                            ...
                                            @else
                                                <svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z"/><path d="M6 12h12M10 6h4M10 10h4M10 14h4M10 18h4"/>
                                                </svg>
                                            @endif
                                        </span>
                                        <div>
                                            <h2 id="address-{{ $address['id'] }}-label" class="font-display text-lg font-bold text-navy-900" data-address-label-el>{{ $address['label'] }}</h2>
                                            <p class="text-sm text-navy-500" data-address-name-el>{{ $address['name'] }}</p>
                                        </div>
                                    </div>
                                    @if ($address['default'])
                                ...
                                <address class="mt-4 not-italic text-sm leading-relaxed text-navy-700" data-address-text>
                                    {{ $address['line1'] }}<br>
                                    @if ($address['line2'])
                                        {{ $address['line2'] }}<br>
                                    @endif
                                ...
                                <div class="mt-auto flex flex-wrap items-center gap-2 pt-6">
                                    ...
                                    <div class="ml-auto flex gap-2">
                                        <button type="button" data-address-edit aria-label="Edit {{ $address['label'] }} address"
                                        ...
                                        <button type="button" data-address-delete aria-label="Delete {{ $address['label'] }} address"
                    ...
                    <button type="button" data-address-add-card
                        ...
                        <span class="flex size-14 items-center justify-center rounded-full bg-navy-900/5 text-navy-700 transition-colors duration-200 group-hover:bg-bronze-500/10">
                            <svg class="size-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true">
                                <path d="M12 5v14M5 12h14"/>
                            </svg>
                        </span>
                        <span>
                            <span class="block font-display text-lg font-bold text-navy-900">Add address</span>
                            <span class="mt-1 block text-sm text-navy-500">Save a new shipping or billing location</span>
                        </span>
                    </button>
                </div>

                {{-- Empty state (hidden until all addresses removed) --}}
                <div data-addresses-empty hidden class="mt-8 rounded-card border border-navy-100 bg-surface p-12 text-center shadow-soft">
                    ...
                    <p class="mx-auto mt-2 max-w-sm text-sm text-navy-600">Add your first address to speed up checkout and keep deliveries on track.</p>
                    <x-ui.button variant="primary" size="sm" data-address-add class="mt-6">Add your first address</x-ui.button>
                </div>
        ...
        {{-- Add / Edit dialog --}}
        <dialog data-address-dialog class="w-full max-w-lg rounded-card border-0 bg-surface p-0 shadow-glass backdrop:bg-navy-900/40 open:animate-scale-in">
            <form method="dialog" class="p-7" data-address-form novalidate>
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="font-display text-xl font-bold text-navy-900" data-address-dialog-title>Add address</h2>
                        <p class="mt-1 text-sm text-navy-500">We’ll use this for shipping unless you choose otherwise at checkout.</p>
                    </div>
                ...
                <div class="mt-6 space-y-4">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="sm:col-span-2">
                            <label for="address-label" class="block text-sm font-medium text-navy-900">Address label</label>
                        ...
                        <div class="sm:col-span-2">
                            <x-ui.input name="full-name" label="Full name" autocomplete="name" value="James Mitchell" required />
                        </div>
                        <div class="sm:col-span-2">
                            <x-ui.input name="line1" label="Street address" autocomplete="address-line1" required />
                        </div>
                        <div class="sm:col-span-2">
                            <x-ui.input name="line2" label="Apt, suite, unit (optional)" autocomplete="address-line2" />
                        </div>
                        ...
                        <div>
                            <label for="address-country" class="block text-sm font-medium text-navy-900">Country</label>
                    ...
                    <div class="overflow-hidden rounded-xl border border-navy-100">
                        <div class="relative h-40 bg-navy-50">
                            <svg viewBox="0 0 480 160" class="block h-full w-full" preserveAspectRatio="xMidYMid slice" role="img" aria-label="Map location picker placeholder">
                                <rect width="480" height="160" class="fill-navy-50"/>
                                <rect x="32" y="24" width="100" height="64" rx="10" class="fill-olive-100"/>
                                <rect x="340" y="80" width="108" height="56" rx="10" class="fill-olive-100"/>
                                <g class="stroke-white" stroke-width="10" stroke-linecap="round">
                                    <path d="M20 80h440"/>
                                    <path d="M160 16v128"/>
                                    <path d="M320 16v128"/>
                                </g>
                                <path d="M240 48c-16 0-28 12-28 28 0 20 28 44 28 44s28-24 28-44c0-16-12-28-28-28Z" class="fill-bronze-500"/>
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center bg-navy-900/5">
                                <svg class="size-8 text-navy-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    <path d="M12 21s-6-5.5-6-10a6 6 0 0 1 12 0c0 4.5-6 10-6 10Zm0-7.5a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5Z"/>
                                </svg>
                ...
                <div class="mt-8 flex flex-wrap justify-end gap-3">
                    <x-ui.button variant="outline" size="sm" type="button" data-address-dialog-close>Cancel</x-ui.button>
        ...
        {{-- Delete confirmation --}}
        <dialog data-address-delete-dialog class="w-full max-w-md rounded-card border-0 bg-surface p-0 shadow-glass backdrop:bg-navy-900/40 open:animate-scale-in">
            <div class="p-7">
                ...
                <p class="mt-2 text-sm text-navy-600">This removes <strong data-delete-label>Home</strong> from your saved addresses. You can add it again anytime.</p>
                <div class="mt-8 flex flex-wrap justify-end gap-3">
                    <x-ui.button variant="outline" size="sm" type="button" data-delete-cancel>Keep address</x-ui.button>
                    <x-ui.button variant="primary" size="sm" type="button" data-delete-confirm class="!bg-red-600 hover:!bg-red-700">Delete address</x-ui.button>
                </div>
            </div>
        </dialog>
            ...
            <article data-address-card data-default="false"
                ...
                <div class="relative h-36 overflow-hidden bg-navy-50" aria-hidden="true">
                    <svg viewBox="0 0 400 144" class="block h-full w-full" preserveAspectRatio="xMidYMid slice">
                        ...
                        <path d="M0 112c48-18 72 12 120 0s72 18 120 6v26H0v-32Z" class="fill-navy-100"/>
                        <g class="stroke-white" stroke-width="8" stroke-linecap="round">
                            <path d="M16 72h368"/><path d="M120 12v120"/><path d="M260 12v120"/><path d="M16 36h368"/>
                        </g>
                        <path d="M200 28c-14 0-25 11-25 25 0 18 25 40 25 40s25-22 25-40c0-14-11-25-25-25Z" class="fill-bronze-500"/>
                        <circle cx="200" cy="53" r="7" class="fill-white"/>
                    </svg>
                ...
                <div class="flex flex-1 flex-col p-6">
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <span class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-navy-900/5 text-navy-700" data-address-icon></span>
                            <div>
                    ...
                    <div class="mt-auto flex flex-wrap items-center gap-2 pt-6">
                        <button type="button" data-address-set-default class="rounded-xl px-3 py-2 text-xs font-semibold text-olive-700 transition-colors duration-200 hover:bg-olive-50">Set as default</button>
                        <div class="ml-auto flex gap-2">
                            <button type="button" data-address-edit class="inline-flex items-center gap-1.5 rounded-xl border border-navy-200 bg-surface px-3 py-2 text-xs font-semibold text-navy-900 transition-colors duration-200 hover:border-navy-300 hover:bg-navy-50">
                            ...
                            <button type="button" data-address-delete class="inline-flex items-center gap-1.5 rounded-xl border border-red-200 bg-surface px-3 py-2 text-xs font-semibold text-red-600 transition-colors duration-200 hover:border-red-300 hover:bg-red-50">
                                <svg class="size-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 6h18M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>
                                Delete
                            </button>
                        </div>
                    </div>
