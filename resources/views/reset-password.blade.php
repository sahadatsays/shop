<x-layouts.auth title="Reset Password" description="Choose a new password for your Valor Supply Co. account.">
    ...
    <div class="flex min-h-screen flex-col items-center justify-center px-6 py-12" data-reset-password>
        ...
        <div class="w-full max-w-md animate-fade-in">
            ...
            <form class="mt-8 space-y-5" data-reset-form method="POST" action="{{ route('password.update') }}" novalidate>
                @csrf
                <input type="hidden" name="token" value="{{ $token }}">
                <x-ui.input name="email" type="email" label="Email address" autocomplete="email" :value="old('email', $email)" required />

                <div class="relative">
                    <x-ui.input name="password" type="password" label="New password" autocomplete="new-password" placeholder="At least 8 characters" required />
