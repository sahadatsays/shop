...
<x-layouts.app :title="$title" description="Your Valor Supply Co. dashboard — orders, rewards, saved gear, and account settings in one place.">
            ...
            <div class="space-y-8 lg:col-span-3">
                ...
                <section class="relative overflow-hidden rounded-card bg-navy-900 p-8 text-white sm:p-10" aria-labelledby="welcome-heading">
                    ...
                    <div class="relative">
                        ...
                        <h1 id="welcome-heading" class="mt-2 font-display text-2xl font-bold sm:text-3xl">{{ $welcome['headline'] }}</h1>
                        <p class="mt-3 max-w-lg text-navy-200">
                            @if ($welcome['highlight'])
                                {!! str_replace($welcome['highlight'], '<strong class="font-semibold text-bronze-400">'.$welcome['highlight'].'</strong>', e($welcome['message'])) !!}
                            @else
                                {{ $welcome['message'] }}
                            @endif
                        </p>
                        <div class="mt-6 flex flex-wrap gap-3">
                            @if ($welcome['show_track_cta'])
                                <x-ui.button :href="$welcome['track_url']" variant="accent" size="sm">Track my order</x-ui.button>
                            @endif
                            <x-ui.button :href="$welcome['shop_url']" size="sm" class="bg-white/10! text-white! hover:bg-white/20!">Shop new arrivals</x-ui.button>
                        </div>
                    </div>
                </section>
                ...
                <div class="grid grid-cols-1 gap-8 xl:grid-cols-5">
                    ...
                    <section class="rounded-card bg-surface p-7 shadow-soft xl:col-span-3" aria-labelledby="orders-heading">
                        ...
                        @if ($recentOrders === [])
                            <div class="mt-8 rounded-xl border border-dashed border-navy-200 bg-navy-50/50 px-6 py-10 text-center">
                                <p class="font-display text-lg font-bold text-navy-900">No orders yet</p>
                                <p class="mt-2 text-sm text-navy-600">When you place your first order, it will show up here with tracking and details.</p>
                                <x-ui.button :href="route('shop')" variant="primary" size="sm" class="mt-6">Start shopping</x-ui.button>
                            </div>
                        @else
                            <ul class="mt-5 divide-y divide-navy-100">
                                @foreach ($recentOrders as $order)
                                    <li class="flex items-center gap-4 py-4">
                                        @if ($order['thumbnail'])
                                            <img src="{{ $order['thumbnail'] }}" alt="" loading="lazy" class="size-12 shrink-0 rounded-xl object-cover bg-navy-100">
                                        @else
                                            <span class="flex size-12 shrink-0 items-center justify-center rounded-xl bg-navy-100 text-navy-400" aria-hidden="true">
                                                <svg class="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M6 7h12l1.2 12.2a1.5 1.5 0 0 1-1.5 1.8H6.3a1.5 1.5 0 0 1-1.5-1.8L6 7Z"/></svg>
                                            </span>
                                        @endif
                                        <div class="min-w-0 flex-1">
                                            <p class="flex flex-wrap items-center gap-x-2 gap-y-1">
                                                <a href="{{ $order['track_url'] }}" class="text-sm font-semibold text-navy-900 hover:text-olive-700">{{ $order['number'] }}</a>
                                                <x-ui.badge :variant="$order['status_variant']">{{ $order['status'] }}</x-ui.badge>
                                            </p>
                                            <p class="mt-0.5 text-xs text-navy-500">{{ $order['placed'] }} · {{ $order['item_count'] }} {{ $order['item_count'] === 1 ? 'item' : 'items' }}</p>
                                        </div>
                                        <p class="text-sm font-bold text-navy-900 tabular-nums">{{ $order['total'] }}</p>
                                        <a href="{{ $order['track_url'] }}" aria-label="View order {{ $order['number'] }}"
                    ...
                    <div class="space-y-8 xl:col-span-2">
                        ...
                        <section class="rounded-card bg-surface p-7 shadow-soft" aria-labelledby="tracking-heading">
                            <div class="flex items-center justify-between gap-4">
                                ...
                                @if ($spotlightOrder)
                                    <a href="{{ $spotlightOrder['track_url'] }}" class="text-xs font-semibold text-navy-500 hover:text-olive-700">{{ $spotlightOrder['number'] }}</a>
                                @endif
                            </div>
                            ...
                            @if ($spotlightOrder)
                                ...
                                <x-ui.button :href="$spotlightOrder['track_url']" variant="outline" size="sm" class="mt-6 w-full">View order details</x-ui.button>
                            @else
                                <div class="mt-6 rounded-xl border border-dashed border-navy-200 bg-navy-50/40 px-5 py-8 text-center">
                        ...
                        <section class="overflow-hidden rounded-card bg-linear-to-br from-bronze-500 to-bronze-700 p-7 text-white shadow-card" aria-labelledby="rewards-heading">
                            ...
                            <p class="mt-4 font-display text-4xl font-extrabold">{{ $rewards['points_label'] }} <span class="text-base font-semibold text-white/70">pts</span></p>
                            <p class="mt-1 text-sm text-white/80">≈ {{ $rewards['redeemable_value'] }} toward your next order</p>
                            <div class="mt-5">
                                <div class="flex items-center justify-between text-xs font-medium text-white/80">
                                    ...
                                    @if ($rewards['next_tier'])
                                        <span>{{ $rewards['next_tier'] }} at {{ number_format($rewards['next_tier_threshold']) }}</span>
                                    @else
                                        <span>Top tier unlocked</span>
                                    @endif
                                </div>
                                <div class="mt-2 h-2 overflow-hidden rounded-full bg-white/25">
                                    <div class="h-full rounded-full bg-white" style="width: {{ $rewards['progress_percent'] }}%" aria-hidden="true"></div>
                                </div>
                            </div>
                            <x-ui.button :href="route('support')" size="sm" class="mt-6 w-full bg-white! text-bronze-700! hover:bg-bronze-50!">Redeem points</x-ui.button>
                        </section>
                    </div>
                </div>
