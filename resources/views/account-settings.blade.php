@php
    $notificationPrefs = [
        [
            'id' => 'notify-orders',
            'label' => 'Order updates',
            'desc' => 'Shipping, delivery, and return confirmations.',
            'checked' => true,
        ],
        [
            'id' => 'notify-promotions',
            'label' => 'Promotions & new arrivals',
            'desc' => 'Sales, limited drops, and seasonal collections.',
            'checked' => true,
        ],
        [
            'id' => 'notify-rewards',
            'label' => 'Rewards & points',
            'desc' => 'Tier changes, point balances, and redemption reminders.',
            'checked' => true,
        ],
        [
            'id' => 'notify-reviews',
            'label' => 'Review reminders',
            'desc' => 'Prompts to review products after delivery.',
            'checked' => false,
        ],
        [
            'id' => 'notify-newsletter',
            'label' => 'Newsletter',
            'desc' => 'Veteran stories, field guides, and brand news.',
            'checked' => true,
        ],
    ];
@endphp
...
<x-layouts.app title="Profile Settings"
    description="Manage your Jackpot BD LTD account — personal details, security, preferences, and notifications.">
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14" data-profile>
        ...
        <div class="grid grid-cols-1 gap-10 lg:grid-cols-4">
            ...
            <div class="lg:col-span-3">
                ...
                <form class="mt-8 space-y-8" method="POST" action="{{ route('profile.update') }}"
                    enctype="multipart/form-data" novalidate data-profile-form>
                    @csrf
                    @method('PUT')

                    {{-- Avatar + Personal information --}}
                    <section class="rounded-card bg-surface p-7 shadow-soft" aria-labelledby="personal-heading">
                        <h2 id="personal-heading" class="font-display text-lg font-bold text-navy-900">Personal
                            information</h2>
                            ...
                            <div class="relative">
                                @if ($customer->avatarUrl())
                                    <span data-avatar-preview
                                        class="flex size-24 items-center justify-center overflow-hidden rounded-full bg-bronze-500 font-display text-3xl font-bold text-white ring-4 ring-navy-100">
                                        <img src="{{ $customer->avatarUrl() }}" alt="{{ $customer->name }}"
                                            class="size-full object-cover">
                                    </span>
                                @else
                                    <span data-avatar-preview
                                        class="flex size-24 items-center justify-center overflow-hidden rounded-full bg-bronze-500 font-display text-3xl font-bold text-white ring-4 ring-navy-100">{{ $customer->initials() }}</span>
                                @endif
                                <label for="avatar"
                                    class="absolute -right-1 -bottom-1 flex size-9 cursor-pointer items-center justify-center rounded-full bg-navy-900 text-white shadow-soft transition-colors duration-200 hover:bg-navy-800">
                                ...
                                <input type="file" id="avatar" name="avatar"
                                    accept="image/jpeg,image/png,image/webp" class="sr-only" data-avatar-input>
                            </div>
                            <div>
                                <p class="text-sm font-semibold text-navy-900">Profile photo</p>
                                <p class="mt-1 text-sm text-navy-500">JPG, PNG, or WebP. Max 2 MB. Square works best.
                                </p>
                                <button type="button" data-avatar-remove hidden
                                    class="mt-3 text-sm font-medium text-red-600 underline-offset-4 hover:underline">Remove
                                    photo</button>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <x-ui.input name="first-name" label="First name" autocomplete="given-name"
                                :value="$firstName" />
                            <x-ui.input name="last-name" label="Last name" autocomplete="family-name"
                                :value="$lastName" />
                            <div class="sm:col-span-2">
                                <x-ui.input name="display-name" label="Display name" :value="$customer->name"
                                    hint="Shown on reviews and your public profile." readonly />
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="rounded-card bg-surface p-7 shadow-soft" aria-labelledby="phone-heading">
                        ...
                        <div class="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <x-ui.input name="phone" type="tel" label="Mobile number" autocomplete="tel"
                                :value="$customer->phone" />
                            <div class="space-y-1.5">
                                <label for="phone-country" class="block text-sm font-medium text-navy-900">Country
                                    code</label>
                    ...
                    <section class="rounded-card bg-surface p-7 shadow-soft" aria-labelledby="password-heading">
                        ...
                        <p class="mt-1 text-sm text-navy-500">Last changed 3 months ago. Use a strong, unique password.
                        </p>
                        <div class="mt-6 space-y-4">
                            <div class="relative">
                                <x-ui.input name="current-password" type="password" label="Current password"
                                ...
                                <button type="button" data-toggle-password="current-password"
                                    ...
                                    <svg data-eye-closed hidden class="size-4.5" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="1.8" stroke-linecap="round"
                                        stroke-linejoin="round" aria-hidden="true">
                                        <path
                                            d="M3 3l18 18M10.6 10.6a2 2 0 0 0 2.8 2.8M9.9 5.1A10.8 10.8 0 0 1 12 5c6.5 0 10 7 10 7a18.2 18.2 0 0 1-4.1 5.2M6.1 6.1C3.3 8 1 12 1 12s3.5 7 10 7c1.2 0 2.3-.2 3.4-.5" />
                                    </svg>
                                </button>
                            </div>
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div class="relative">
                                    <x-ui.input name="new-password" type="password" label="New password"
                                    ...
                                    <button type="button" data-toggle-password="new-password"
                                        ...
                                        <svg data-eye-closed hidden class="size-4.5" viewBox="0 0 24 24"
                                ...
                                <div class="relative">
                                    <x-ui.input name="confirm-password" type="password" label="Confirm new password"
                    ...
                    <section class="rounded-card bg-surface p-7 shadow-soft" aria-labelledby="notifications-heading">
                        <h2 id="notifications-heading" class="font-display text-lg font-bold text-navy-900">
                            Notifications</h2>
                        <p class="mt-1 text-sm text-navy-500">Choose what we send and where. You can change this
                            anytime.</p>
                        <ul class="mt-6 divide-y divide-navy-100">
                            @foreach ($notificationPrefs as $pref)
                                <li class="flex items-start justify-between gap-4 py-4 first:pt-0 last:pb-0">
                                    <div>
                                        <label for="{{ $pref['id'] }}"
                                            class="text-sm font-semibold text-navy-900">{{ $pref['label'] }}</label>
                                        <p class="mt-0.5 text-sm text-navy-500">{{ $pref['desc'] }}</p>
                                    </div>
                                    <label class="relative inline-flex shrink-0 cursor-pointer items-center">
                                        <input type="checkbox" id="{{ $pref['id'] }}" name="{{ $pref['id'] }}"
                                            @checked($pref['checked']) class="peer sr-only">
                                        <span
                                            class="h-6 w-11 rounded-full bg-navy-200 transition-colors duration-200 peer-checked:bg-olive-600 peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-bronze-500"
                                            aria-hidden="true"></span>
                                        <span
                                            class="absolute left-0.5 size-5 rounded-full bg-white shadow-soft transition-transform duration-200 peer-checked:translate-x-5"
                                            aria-hidden="true"></span>
                                    </label>
                                </li>
                            @endforeach
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-4 rounded-card bg-navy-50 p-6">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <x-ui.button type="button" variant="ghost" data-profile-reset>Discard
                                changes</x-ui.button>
