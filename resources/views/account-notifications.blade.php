...
<x-layouts.app title="Notifications" description="Your Valor Supply Co. notification center — orders, offers, rewards, and account updates in one place.">
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14" data-notifications data-mark-all-url="{{ route('account.notifications.mark-all-read') }}">
            ...
            <div class="lg:col-span-3">
                ...
                <div class="mt-6 flex flex-wrap gap-2" role="group" aria-label="Filter notifications">
                    @foreach ($filters as $filter)
                        @php
                            $isActive = strtolower($filter) === $filterKey || ($filter === 'All' && $filterKey === 'all');
                            $href = $filter === 'All'
                                ? route('account.notifications')
                                : route('account.notifications', ['filter' => strtolower($filter)]);
                        @endphp
                        <a href="{{ $href }}"
                           data-notifications-filter="{{ $filter }}"
                ...
                <div class="mt-8 space-y-10" data-notifications-list>
                                    ...
                                    >
                                        ...
                                        <div data-notification-card
                                            ...
                                            <div class="flex gap-4">
                                                <span class="flex size-11 shrink-0 items-center justify-center rounded-xl {{ $iconColors[$type] ?? $iconColors['account'] }}">
                                                ...
                                                <div class="min-w-0 flex-1">
                                                    <div class="flex flex-wrap items-start justify-between gap-2">
                                                        ...
                                                        <time class="shrink-0 text-xs text-navy-400">{{ $notification->timeAgo() }}</time>
                                                    </div>
                    ...
                    @empty
                        <div class="rounded-card border border-navy-100 bg-surface p-12 text-center shadow-soft">
                            ...
                            <p class="mx-auto mt-2 max-w-sm text-sm text-navy-600">Order updates, promotions, and account alerts will show up here.</p>
                        </div>
                    @endforelse
                </div>
                ...
                <div data-notifications-filter-empty hidden class="mt-8 rounded-card border border-navy-100 bg-surface p-12 text-center shadow-soft">
                    ...
                    <a href="{{ route('account.notifications') }}" class="mt-6 inline-block text-sm font-semibold text-bronze-600 underline-offset-4 hover:underline">Show all notifications</a>
                </div>

                <div data-notifications-all-read @if ($unreadCount > 0) hidden @endif class="mt-8 rounded-card border border-olive-200/60 bg-olive-50/40 p-6 text-center shadow-soft">
