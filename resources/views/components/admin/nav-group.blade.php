...
<li data-nav-group>
    <button type="button"
            ...
            title="{{ $item->label }}"
            @class([
                'admin-nav-link flex w-full items-center gap-3 rounded-[var(--radius-admin)] px-3 py-2.5 text-sm font-medium admin-focus-ring transition-colors',
        ...
        <svg class="admin-nav-label ml-auto size-4 shrink-0 admin-muted transition-transform [[aria-expanded=true]_&]:rotate-90" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
            <path d="m9 18 6-6-6-6"/>
        </svg>
    </button>

    <ul data-nav-group-panel @if(! $expanded) hidden @endif class="admin-nav-group-panel mt-1 space-y-0.5 pl-9">
