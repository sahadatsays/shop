...
<x-admin.field :label="$label" :name="$name" :help="$help" :required="$required" :class="$wrapperClass">
    <textarea name="{{ $name }}" id="{{ $inputId }}" rows="{{ $rows }}"
        ...
        {{ $attributes->except('class')->merge([
            'class' =>
                'block w-full rounded-[var(--radius-admin)] border bg-admin-bg px-3.5 py-2.5 text-sm admin-text placeholder:admin-muted admin-focus-ring ' .
                ($hasError ? 'border-admin-danger' : 'admin-border'),
        ]) }}>{{ $value }}</textarea>
</x-admin.field>
