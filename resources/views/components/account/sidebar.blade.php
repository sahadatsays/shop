...
@php
    $navItems = [
        ...
        ['label' => 'Wishlist', 'route' => 'wishlist', 'badge' => ($wishlistItemCount ?? 0) > 0 ? (string) ($wishlistItemCount ?? 0) : null, 'icon' => 'M12 21s-7.5-4.7-9.5-9A5.5 5.5 0 0 1 12 6.5 5.5 5.5 0 0 1 21.5 12c-2 4.3-9.5 9-9.5 9Z'],
        ...
        ['label' => 'Notifications', 'route' => 'account.notifications', 'badge' => ($customerUnreadNotificationCount ?? 0) > 0 ? (string) ($customerUnreadNotificationCount ?? 0) : null, 'icon' => 'M6 9a6 6 0 0 1 12 0c0 5 2 6 2 6H4s2-1 2-6M10 19a2 2 0 0 0 4 0'],
        ['label' => 'Settings', 'route' => 'account.settings', 'badge' => null, 'icon' => 'M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Zm7.5-3a7.5 7.5 0 0 0-.1-1.2l2-1.6-2-3.4-2.4 1a7.6 7.6 0 0 0-2-1.2L14.6 3h-4l-.4 2.6a7.6 7.6 0 0 0-2 1.2l-2.4-1-2 3.4 2 1.6a7.7 7.7 0 0 0 0 2.4l-2 1.6 2 3.4 2.4-1a7.6 7.6 0 0 0 2 1.2l.4 2.6h4l.4-2.6a7.6 7.6 0 0 0 2-1.2l2.4 1 2-3.4-2-1.6c.06-.4.1-.8.1-1.2Z'],
    ];
@endphp
        ...
        <ul class="flex gap-1 overflow-x-auto scrollbar-none lg:flex-col lg:overflow-visible">
            @foreach ($navItems as $item)
                ...
                <li class="shrink-0 lg:shrink">
                    <a href="{{ $item['route'] ? route($item['route']) : '#' }}" @if ($isActive) aria-current="page" @endif
                       class="flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-medium whitespace-nowrap transition-colors duration-200
                        ...
                        <svg class="size-4.5 shrink-0 {{ $isActive ? 'text-bronze-400' : 'text-navy-400' }}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                            <path d="{{ $item['icon'] }}"/>
                        </svg>
                        {{ $item['label'] }}
                        @if ($item['badge'])
