...
<article
    {{ $attributes->merge(['class' => 'group relative flex flex-col overflow-hidden rounded-card bg-surface shadow-card transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-card-hover']) }}>
    <div class="relative aspect-4/5 overflow-hidden bg-navy-100">
        ...
        @else
            <div
                class="flex size-full items-center justify-center bg-linear-to-br from-navy-100 via-navy-50 to-bronze-100 transition-transform duration-500 ease-out group-hover:scale-105">
                <svg class="size-14 text-navy-300" viewBox="0 0 24 24" fill="none" stroke="currentColor"
        ...
        <div class="absolute top-3 right-3 z-10 flex flex-col gap-2">
            @if ($productId)
                ...
                <button type="button" data-wishlist-toggle data-product-id="{{ $productId }}"
            ...
            @if ($productId)
                ...
                <button type="button" data-compare-toggle data-product-id="{{ $productId }}"
        ...
        <div class="absolute inset-x-3 bottom-3 z-10">
            <button type="button"
                ...
                @if ($productId) data-add-to-cart
                        data-product-id="{{ $productId }}" @endif>
            ...
            <div class="mt-1">
                @if ($stockPercent !== null)
                    <div class="h-1.5 overflow-hidden rounded-full bg-navy-100">
                        <div class="h-full rounded-full {{ $stockPercent <= 25 ? 'bg-red-500' : 'bg-olive-600' }}"
                            style="width: {{ $stockPercent }}%"></div>
                    </div>
                @endif
                <p
                    class="mt-1.5 text-xs font-medium {{ $stockPercent !== null && $stockPercent <= 25 ? 'text-red-600' : 'text-olive-700' }}">
                    {{ $stock }}</p>
            </div>
        @endif
    </div>
