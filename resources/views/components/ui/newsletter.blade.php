...
<section {{ $attributes->merge(['class' => 'mx-auto max-w-7xl px-4 sm:px-6 lg:px-8']) }} data-reveal data-newsletter>
    <div
        class="relative overflow-hidden rounded-card bg-linear-to-br from-olive-800 to-olive-600 px-6 py-16 shadow-card sm:px-12 lg:px-16">
        ...
        <div class="relative mx-auto max-w-xl text-center">
            <h2 class="font-display text-3xl font-bold text-white sm:text-4xl">Newsletter</h2>
            <p class="mt-4 text-lg text-olive-100">
                Get updates on new products, exclusive offers, custom packaging ideas, and the latest from Jackpot
                Accessories.
            </p>
            <form class="mt-8 flex flex-col gap-3 sm:flex-row" method="post"
                action="{{ $action ?? route('newsletter.subscribe') }}" data-newsletter-form>
                @csrf
            ...
            <p class="mt-4 text-sm text-olive-200" data-newsletter-status aria-live="polite">No spam. Unsubscribe
                anytime.</p>
        </div>
    </div>
</section>
