...
<section
    class="relative -mt-[var(--site-header-stack,6.5rem)] overflow-hidden bg-navy-950 pt-[var(--site-header-stack,6.5rem)]"
    data-hero-slider aria-roledescription="carousel" aria-label="Hero banners">
    @if ($slides->isEmpty())
        <div class="relative flex min-h-[85vh] items-center">
            <div class="absolute inset-0" aria-hidden="true">
                <img src="{{ $fallbackImage }}" alt="" fetchpriority="high"
                    class="size-full animate-slow-zoom object-cover opacity-60">
            ...
            <div class="relative mx-auto w-full max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
                <div class="max-w-2xl">
                    ...
                    <h1 class="font-display text-4xl font-extrabold tracking-tight text-white sm:text-6xl lg:text-7xl">
                        Honor in every stitch and seam.</h1>
                    <p class="mt-6 max-w-xl text-lg leading-relaxed text-navy-200">Premium apparel, collectibles, and
                        field gear designed by veterans.</p>
                    <div class="mt-10 flex flex-wrap items-center gap-4">
                        <x-ui.button :href="route('shop')" variant="accent" size="lg">Shop best sellers</x-ui.button>
                        <x-ui.button :href="route('about')" size="lg"
                            class="border border-white/20 bg-white/10 text-white backdrop-blur-sm hover:bg-white/20">Our
                            story</x-ui.button>
                    </div>
                </div>
            </div>
            ...
            @foreach ($slides as $index => $banner)
                ...
                <article data-hero-slide @class([
                    ...
                    <div class="absolute inset-0" aria-hidden="true">
                        ...
                        <div class="absolute inset-0 bg-linear-to-r from-navy-950 via-navy-950/70 to-navy-950/20"></div>
                        <div class="absolute -bottom-40 -left-24 size-[30rem] rounded-full bg-olive-600/20 blur-3xl">
                        </div>
                    </div>
                    ...
                    <div class="relative mx-auto w-full max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
                        <div class="max-w-2xl animate-fade-in-up">
                            ...
                            <h1
                                class="font-display text-4xl font-extrabold tracking-tight text-white sm:text-6xl lg:text-7xl">
                                {{ $banner->title }}
                            </h1>
                            ...
                            <div class="mt-10 flex flex-wrap items-center gap-4">
                                ...
                                @if ($banner->secondary_label && $banner->secondary_url)
                                    <x-ui.button :href="$banner->secondary_url" size="lg"
                                        class="border border-white/20 bg-white/10 text-white backdrop-blur-sm hover:bg-white/20">
                    ...
                    @foreach ($slides as $index => $banner)
                        <button type="button" data-hero-dot="{{ $index }}"
                            ...
                            <span data-hero-progress
                                class="block h-full w-0 bg-bronze-400 transition-[width] duration-100 ease-linear"></span>
                        </button>
                    @endforeach
                </div>
