...
<body class="min-h-screen bg-canvas text-ink antialiased">
    ...
    @if ($minimal ?? false)
        ...
        <header class="glass sticky top-0 z-50 border-b border-navy-900/5">
            <div
                class="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:h-[4.5rem] lg:px-8">
                <a href="{{ route('home') }}" class="flex shrink-0 items-center gap-2.5"
                    ...
                    @endif
                    <span
                        class="site-header-brand font-display text-lg font-bold tracking-tight">{{ $storeName }}</span>
                </a>
    ...
    @else
        ...
        <div data-site-header-shell class="sticky top-0 z-50">
            <div
                class="bg-[var(--store-utility-bg,var(--color-navy-950))] text-center text-xs font-medium tracking-wide text-[var(--store-utility-text,var(--color-navy-200))]">
            ...
            <header data-site-header
                ...
                <div
                    class="relative mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:h-[4.5rem] lg:px-8">
                    {{-- Brand --}}
                    <a href="{{ route('home') }}" class="flex shrink-0 items-center gap-2.5"
                        aria-label="{{ $storeName }} — Home">
                        @if ($storeSettings->logoUrl())
                        ...
                        @else
                            <span
                                class="flex size-9 items-center justify-center rounded-xl bg-navy-900 text-bronze-400">
                                <svg class="size-5" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                    <path
                                        d="M12 2 4 5v6c0 5.25 3.4 9.74 8 11 4.6-1.26 8-5.75 8-11V5l-8-3Zm0 4.2 1.4 2.84 3.13.46-2.26 2.2.53 3.12L12 13.35l-2.8 1.47.53-3.12-2.26-2.2 3.13-.46L12 6.2Z" />
                                </svg>
                    ...
                    <nav class="hidden items-center gap-1 lg:flex" aria-label="Main navigation">
                            ...
                            @if ($menuItem->children->isNotEmpty())
                                <div class="group/mega">
                                    <button type="button" aria-haspopup="true" aria-expanded="false"
                                        ...
                                        {{ $menuItem->label }}
                                        <svg class="size-3.5 transition-transform duration-200 group-hover/mega:rotate-180"
                                    ...
                                    <div
                                        class="invisible absolute inset-x-0 top-full translate-y-2 pt-2 opacity-0 transition-all duration-200 ease-out group-focus-within/mega:visible group-focus-within/mega:translate-y-0 group-focus-within/mega:opacity-100 group-hover/mega:visible group-hover/mega:translate-y-0 group-hover/mega:opacity-100">
                                        <div
                                            class="mx-auto max-w-7xl overflow-hidden rounded-b-card bg-surface shadow-card-hover ring-1 ring-navy-900/5">
                                                ...
                                                <div class="col-span-2 grid grid-cols-2 gap-1 p-8">
                                                    @foreach ($menuItem->children as $child)
                                                        <a href="{{ $child->resolvedUrl() }}"
                                                            ...
                                                            <span
                                                                class="flex size-11 shrink-0 items-center justify-center rounded-xl bg-olive-100 text-olive-700 transition-colors duration-200 group-hover/item:bg-olive-600 group-hover/item:text-white">
                                                            ...
                                                            <span>
                                                                <span
                                                                    class="block text-sm font-semibold text-navy-900">{{ $child->label }}</span>
                                                            </span>
                                                        </a>
                                                    @endforeach
                                                </div>
                                                <a href="{{ route('shop') }}"
                                                    class="group/feature relative m-4 overflow-hidden rounded-card">
                                                    <img src="https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=800&q=70&auto=format&fit=crop"
                                                        alt="Camper van on an open road at sunset" loading="lazy"
                                                        class="absolute inset-0 size-full object-cover transition-transform duration-500 group-hover/feature:scale-105">
                                                    <span
                                                        class="absolute inset-0 bg-linear-to-t from-navy-950/80 via-navy-950/20 to-transparent"></span>
                                                    <span
                                                        class="relative flex h-full min-h-64 flex-col justify-end p-6">
                                                        <x-ui.badge variant="bronze" class="self-start">New
                                                            season</x-ui.badge>
                                                        <span
                                                            class="mt-3 font-display text-lg font-bold text-white">The
                                                            Expedition Collection</span>
                                                        <span class="mt-1 text-sm text-navy-200">Gear for the long way
                                                            home</span>
                                                    </span>
                                                </a>
                                            </div>
                        ...
                        @empty
                            <a href="{{ route('shop') }}"
                                class="site-header-link rounded-xl px-4 py-2 text-sm font-medium transition-colors duration-200 hover:bg-white/10 hover:text-white">Shop</a>
                            <a href="{{ route('about') }}"
                                class="site-header-link rounded-xl px-4 py-2 text-sm font-medium transition-colors duration-200 hover:bg-white/10 hover:text-white">Our
                                Story</a>
                            <a href="{{ route('support') }}"
                                class="site-header-link rounded-xl px-4 py-2 text-sm font-medium transition-colors duration-200 hover:bg-white/10 hover:text-white">Support</a>
                        @endforelse
                    </nav>

                    {{-- Actions --}}
                    <div class="flex items-center gap-0.5 sm:gap-1">
                        <button type="button" data-search-toggle aria-expanded="false" aria-controls="search-panel"
                        ...
                        <a href="{{ route('wishlist') }}" data-wishlist-link
                            aria-label="Wishlist, {{ $wishlistItemCount ?? 0 }} {{ ($wishlistItemCount ?? 0) === 1 ? 'item' : 'items' }}"
                            ...
                            <span data-wishlist-count @class([
                                'absolute -top-0.5 -right-0.5 flex size-4.5 items-center justify-center rounded-full bg-bronze-500 text-[0.65rem] font-bold text-white',
                                'hidden' => ($wishlistItemCount ?? 0) <= 0,
                            ])>{{ $wishlistItemCount ?? 0 }}</span>
                        </a>
                        <a href="{{ route('compare') }}" data-compare-link
                            ...
                            <span data-compare-count-badge
                                @class([
                                    'absolute -top-0.5 -right-0.5 flex size-4.5 items-center justify-center rounded-full bg-olive-600 text-[0.65rem] font-bold text-white',
                                    'hidden' => ($compareItemCount ?? 0) <= 0,
                                ])>{{ $compareItemCount ?? 0 }}</span>
                        </a>
                        <a href="{{ session('customer_id') ? route('account') : route('login') }}"
                            aria-label="{{ session('customer_id') ? 'Account' : 'Sign in' }}"
                        ...
                        <a href="{{ route('cart') }}" data-cart-link
                            aria-label="Cart, {{ $cartItemCount ?? 0 }} {{ ($cartItemCount ?? 0) === 1 ? 'item' : 'items' }}"
                        ...
                        <button type="button" data-nav-toggle aria-expanded="false" aria-controls="mobile-nav"
                            ...
                            <svg data-icon-open class="size-5" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true">
                                <path d="M4 7h16M4 12h16M4 17h16" />
                            </svg>
                            <svg data-icon-close class="hidden size-5" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true">
                ...
                <div id="search-panel" data-search-panel hidden class="border-t border-navy-900/5">
                    <form action="{{ route('search') }}" method="get"
                        ...
                        <svg class="size-5 shrink-0 text-navy-400" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="1.8" stroke-linecap="round" aria-hidden="true">
                            <circle cx="11" cy="11" r="7" />
                            <path d="m20 20-3.5-3.5" />
                        </svg>
                        <label for="site-search" class="sr-only">Search products</label>
                        <input type="search" id="site-search" name="q" data-search-input
                ...
                {{-- Mobile navigation --}}
                <nav id="mobile-nav" data-mobile-nav hidden
                    class="max-h-[70vh] overflow-y-auto border-t border-navy-900/5 px-4 pt-2 pb-4 lg:hidden"
...
<<<<<<< HEAD
                    ...
                    @empty
                        @foreach (['Shop' => route('shop'), 'Categories' => route('categories'), 'Our Story' => route('about'), 'Support' => route('support')] as $label => $href)
                            <a href="{{ $href }}"
                                class="block rounded-xl px-4 py-3 text-base font-medium text-navy-800 transition-colors duration-200 hover:bg-navy-900/5">{{ $label }}</a>
                        @endforeach
                    @endforelse
                    <a href="{{ route('wishlist') }}"
                        class="block rounded-xl px-4 py-3 text-base font-medium text-navy-800 transition-colors duration-200 hover:bg-navy-900/5">Wishlist</a>
                    <a href="{{ session('customer_id') ? route('account') : route('login') }}"
                        class="block rounded-xl px-4 py-3 text-base font-medium text-navy-800 transition-colors duration-200 hover:bg-navy-900/5">{{ session('customer_id') ? 'Account' : 'Sign in' }}</a>
                </nav>
            </header>
        </div>
=======
...
>>>>>>> origin/main
    ...
    @if ($minimal ?? false)
        ...
        <footer class="mt-20 border-t border-navy-100 bg-surface">
            <div
                class="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 px-4 py-8 sm:px-6 lg:flex-row lg:px-8">
                <p class="text-sm text-navy-500">&copy; {{ date('Y') }} {{ $storeName }}. All rights reserved.
                </p>
                <ul class="flex flex-wrap justify-center gap-x-6 gap-y-2">
                    @foreach (['Privacy Policy', 'Terms of Service', 'Refund Policy', 'Help'] as $link)
                        <li><a href="#"
                                class="text-sm text-navy-500 transition-colors duration-200 hover:text-navy-900">{{ $link }}</a>
                        </li>
                    @endforeach
                </ul>
    ...
    @else
                ...
                <div class="grid grid-cols-1 gap-12 lg:grid-cols-6">
                    <div class="space-y-5 lg:col-span-2">
                        <div class="flex items-center gap-2.5">
                            @if ($storeSettings->logoUrl())
                                <img src="{{ $storeSettings->logoUrl() }}" alt=""
                                    class="h-10 w-auto rounded-lg">
                            @else
                                <span
                                    class="flex size-10 items-center justify-center rounded-xl bg-white/10 text-bronze-400">
                                    <svg class="size-5.5" viewBox="0 0 24 24" fill="currentColor"
                                        aria-hidden="true">
                                        <path
                                            d="M12 2 4 5v6c0 5.25 3.4 9.74 8 11 4.6-1.26 8-5.75 8-11V5l-8-3Zm0 4.2 1.4 2.84 3.13.46-2.26 2.2.53 3.12L12 13.35l-2.8 1.47.53-3.12-2.26-2.2 3.13-.46L12 6.2Z" />
                                    </svg>
                        ...
                        <ul class="space-y-2 text-sm text-navy-300">
                            <li class="flex items-center gap-2.5">
                                <svg class="size-4 text-bronze-400" viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor" stroke-width="1.8" stroke-linecap="round"
                                    stroke-linejoin="round" aria-hidden="true">
                                    <path d="M4 6h16v12H4zM4 7l8 6 8-6" />
                                </svg>
                                <a href="mailto:{{ $storeSettings->contactEmail() }}"
                                    class="transition-colors hover:text-[var(--store-link-accent,var(--color-bronze-400))]">{{ $storeSettings->contactEmail() }}</a>
                            </li>
        ...
        'YouTube' => ['url' => $storeSettings->socialLinks()['youtube'], 'path' => 'M21 8s-.2-1.4-.8-2c-.8-.8-1.6-.8-2-.9C15.4 5 12 5 12 5s-3.4 0-6.2.1c-.4.1-1.2.1-2 .9-.6.6-.8 2-.8 2S3 9.6 3 11.2v1.5C3 14.4 3.2 16 3.2 16s.2 1.4.8 2c.8.8 1.8.8 2.2.9 1.6.1 5.8.1 5.8.1s3.4 0 6.2-.1c.4-.1 1.2-.1 2-.9.6-.6.8-2 .8-2s.2-1.6.2-3.3v-1.5C21.2 9.6 21 8 21 8ZM10 14.6V9.4l5 2.6-5 2.6Z'],
                                ...
                                @if ($social['url'])
                                    <a href="{{ $social['url'] }}" aria-label="{{ $network }}" target="_blank"
                    ...
                    @forelse ($footerMenus ?? [] as $footerMenu)
                        <nav aria-label="{{ $footerMenu['name'] }} links">
                            <h3 class="font-display text-sm font-semibold tracking-wide text-white uppercase">
                                {{ $footerMenu['name'] }}</h3>
        ...
        'Legal' => [['Privacy Policy', '#']],
                            ...
                            <nav aria-label="{{ $heading }} links">
                                ...
                                <ul class="mt-5 space-y-3">
                                    @foreach ($links as [$label, $href])
                                        <li><a href="{{ $href }}"
                                                class="text-sm text-navy-300 transition-colors duration-200 hover:text-bronze-400">{{ $label }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </nav>
                ...
                <div
                    class="mt-16 flex flex-col items-center justify-between gap-6 border-t border-white/10 pt-8 lg:flex-row">
                    <p class="text-sm text-navy-400">&copy; {{ date('Y') }} {{ $storeName }}. All rights
                        reserved.</p>
                    <ul class="flex flex-wrap justify-center gap-2">
                        @foreach (['VISA', 'Mastercard', 'AMEX', 'PayPal', 'Apple Pay', 'G Pay'] as $method)
                            <li
                                class="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs font-bold tracking-wide text-navy-300">
                                {{ $method }}</li>
                        @endforeach
                    </ul>
                    <p class="flex items-center gap-2 text-sm text-navy-400">
                        <svg class="size-4 text-bronze-400" viewBox="0 0 24 24" fill="currentColor"
                            aria-hidden="true">
                            <path d="M12 2 4 5v6c0 5.25 3.4 9.74 8 11 4.6-1.26 8-5.75 8-11V5l-8-3Z" />
                        </svg>
                        {{ $storeTagline }}
