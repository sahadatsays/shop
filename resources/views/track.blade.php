...
<x-layouts.app title="Track Shipment" description="Follow your Valor Supply Co. order in real time — live status, courier details, and estimated arrival.">
    ...
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14" data-track>
        ...
        <section class="relative mt-8 overflow-hidden rounded-card bg-navy-900 p-8 text-white sm:p-10" aria-labelledby="eta-heading">
            <div class="absolute -top-24 -right-16 size-64 rounded-full bg-olive-600/30 blur-3xl" aria-hidden="true"></div>
            <div class="absolute -bottom-24 right-40 size-52 rounded-full bg-bronze-500/20 blur-3xl" aria-hidden="true"></div>
        ...
        <nav aria-label="Shipment milestones" class="mt-6 overflow-x-auto rounded-card bg-surface p-5 shadow-soft scrollbar-none">
            <ol class="flex min-w-max items-center gap-0 px-2 sm:min-w-0 sm:justify-between">
                @foreach ($timeline as $step)
                    ...
                    <li class="flex flex-col items-center gap-2 px-1">
                        <span class="flex size-7 items-center justify-center rounded-full sm:size-8
                        ...
                        <span class="max-w-16 text-center text-[0.65rem] font-semibold leading-tight sm:max-w-none sm:text-xs {{ $step['state'] === 'upcoming' ? 'text-navy-400' : 'text-navy-900' }}">{{ $step['label'] }}</span>
                    </li>
                @endforeach
            </ol>
        </nav>
                ...
                <ol class="mt-6">
                    @foreach ($timeline as $step)
                        <li class="relative flex gap-4 {{ $loop->last ? '' : 'pb-8' }}">
                            ...
                            <div class="pt-1">
                                <p class="text-sm font-semibold {{ $step['state'] === 'upcoming' ? 'text-navy-400' : 'text-navy-900' }}">
                                    {{ $step['label'] }}
                ...
                <section class="overflow-hidden rounded-card bg-surface shadow-soft" aria-label="Delivery route map">
                    <div class="relative">
                            ...
                            <g>
                                ...
                                <g class="fill-bronze-400">
                                    <path d="M-7-3h8v6h-8zM1-1h4l2 2v2H1z" transform="scale(1.1)">
                                        <animateMotion dur="14s" repeatCount="indefinite" keyPoints="0;0.72;0.72;1;1" keyTimes="0;0.55;0.7;0.95;1" calcMode="linear">
                                            <mpath href="#route"/>
                                        </animateMotion>
                                    </path>
                                </g>
                        ...
                        <div class="absolute bottom-4 left-4 flex flex-wrap gap-x-5 gap-y-1.5 rounded-xl bg-white/90 px-4 py-2.5 text-xs font-medium text-navy-700 shadow-soft backdrop-blur-sm">
                            ...
                            <span class="flex items-center gap-2"><span class="size-2.5 rounded-full bg-bronze-500" aria-hidden="true"></span>Your address</span>
                        </div>
                    </div>
                </section>
                ...
                <div class="grid grid-cols-1 gap-8 sm:grid-cols-2">
                    ...
                    <section class="rounded-card bg-surface p-7 shadow-soft" aria-labelledby="courier-heading">
                        ...
                        <div class="mt-5 flex items-center gap-4">
                            <span class="flex size-12 shrink-0 items-center justify-center rounded-xl bg-navy-900 text-bronze-400">
                                <svg class="size-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    <path d="M3 7h11v8H3zM14 10h4l3 3v2h-7zM7 18a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Zm11 0a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z"/>
                                </svg>
                            </span>
                            <div>
